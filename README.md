...
```
Usage:
  get_tvrain [-h|--long-help] [-v] [-F] [-S] [-c <merge|save|pls|info>]
...
Options:
  ...
  -c Command, instructs get_tvrain what to do:
       m[erge] - downloads all parts of the video and merges them into one
                 matroska file, requires temp directory for downloaded parts,
                 this is the default command
       s[ave]  - saves each part of the video as is
       p[ls]   - does not download any video, saves a playlist with video URLs
       i[nfo]  - print out video informaton
  -q Video quality selector, available options are:
       f[hd] - full HD aka 1080p
       h[d]  - high definition aka 720p (default)
       s[d]  - standard definition aka 480p
       l[o]  - low bitrate aka 360p
